...
const GroupMemberList = ({ grpId, grpDetail, setGrpDetail }) => {
  ...
  const mutation = useMutation({
    mutationFn: async (userId) => {
      const response = await fetch(
        "http://127.0.0.1:8080/group/removeFromGroup",
        {
          method: "put",
  ...
  const addMember = useMutation({
    mutationFn: async (userId) => {
      const response = await fetch(
        "http://127.0.0.1:8080/group/addGroupMember",
        {
          method: "put",
  ...
  const GroupMember = useQuery("getGroupMember", async () => {
    ...
    const response = await fetch("http://127.0.0.1:8080/group/getGroupMember", {
      method: "post",
  ...
  const changeLeaderClick = async (grpId, rshId) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const response = await fetch(
          "http://127.0.0.1:8080/group/changeLeaderGroup",
          {
            method: "post",
