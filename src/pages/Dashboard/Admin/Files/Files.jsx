...
const Files = () => {
  ...
  const { data } = useQuery({
    queryFn: async () => {
      const response = await fetch("http://127.0.0.1:8080/files/list", {
        method: "get",
        credentials: "include",
      });
  ...
  const filesUpload = useMutation({
    mutationFn: async (formData) => {
      const response = await fetch("http://127.0.0.1:8080/files/fileUplaod", {
        method: "post",
        body: formData,
        credentials: "include",
  ...
  const fileDelete = useMutation({
    mutationFn: async (id) => {
      const response = await fetch("http://127.0.0.1:8080/files/delete", {
        method: "post",
