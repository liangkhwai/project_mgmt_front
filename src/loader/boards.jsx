...
export async function checkBoards(grpId) {
  ...
  const getBoards = async () => {
    const response = await fetch(
      `${process.env.REACT_APP_BOARD_GROUP_GET}${localStorage.getItem(
        "grpId",
      )}`,
      {
        method: "get",
  ...
  const getGroup = async () => {
    const response = await fetch(process.env.REACT_APP_GROUP_GET, {
      method: "get",
      credentials: "include",
    });
  ...
  if (
    localStorage.getItem("grpId") === null ||
    localStorage.getItem("grpId") === undefined
  ) {
    console.log(localStorage.getItem("grpId"));
    console.log("+++");
    boardsCheck = false;
